...
def write_xlsx(self, file_dict, path):
    ...
    chartRowIndex = {}
    for tab_key, species_dict in file_dict.iteritems():
        if tab_key == 'Time':
            continue
        tb = wb.add_worksheet(tab_key)
        if tab_key == 'Parameterization':
            _row = -1
            for col_1, col_2 in species_dict.iteritems():
                _row += 1
            ...
            _row = 0
            for species, itemList in species_dict.iteritems():
                _row += 1
        ...
        for species, group_dict in species_dict.iteritems():
            if len(species) > spLen:
                spLen = len(species)
            for group, name_dict in group_dict.iteritems():
                if len(group) > gpLen:
                    gpLen = len(group)
                for name, itemList in name_dict.iteritems():
                    if len(name) > nameLen:
                        nameLen = len(name)
                    row += 1
                    if group != 'NA':
    ...
    try:
        # Add charts
        chartItems = []
        for species, chartRows in chartRowIndex.iteritems():
            chartItems += [[('Total Population', '=Population!$D${}:${}${}'.format(
                          ...
                          [('{} Female Deaths'.format(gp), '=Mortality!$D${}:${}${}'.format(
                           ...
                           [('Total {} Deaths'.format(mm), '=Mortality!$D${}:${}${}'.format(
                             ...
                             '=Mortality!$D$1:${}$1'.format(index_to_char(chartRows['All deaths'][1])),
                             'Deaths', '{} Total Mortality by Mortality Type (anthro, natural)'.format(species))\
                            for mm in numpy.unique(self.mortalityNames).tolist() + ['Old Age', 'Density Dependent']],
                           [('Total Male Deaths', '=Mortality!$D${}:${}${}'.format(
                               chartRows['Total male deaths'][0],
                               index_to_char(chartRows['Total male deaths'][1]),
                               chartRows['Total male deaths'][0]),
