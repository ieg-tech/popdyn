...
def dispersive_flux(self, species, t, population, k, distance, file_instance,
    ...
    dist = numpy.ones(shape=window, dtype='bool')
    ...
    if relocate_distance > 0:
        ...
        reloc_dist = numpy.ones(shape=reloc_window, dtype='bool')
        ...
        index_sort = numpy.argsort(numpy.rec.fromarrays([max_i, max_j]), order=('f0', 'f1'))
        p_i = p_i[index_sort]
        p_j = p_j[index_sort]
        max_i = max_i[index_sort]
        max_j = max_j[index_sort]
        # Find locations of duplicates
        duplicates = numpy.concatenate([[0], (numpy.diff(max_i) == 0) & (numpy.diff(max_j) == 0)])
        ...
        if duplicates.sum() != 0:
            ...
            pop_add = numpy.bincount(bins, dup_pop)[1:]
            # Replace values, as numpy only uses one index based on ordering
    ...
    if numpy.unique(dist[dm]).size > 1:
        _m, _b = numpy.linalg.solve([[dist.max(), 1.], [dist[dm].min(), 1.]], [1 / 3., 1.])
    else:
        _m, _b = 2 / 3., 0
    ...
    overalloc = numpy.abs(_pos[sub_alloc])
    ...
    for sex in ['male', 'female']:
        for key in file_instance['%s/%s/%s' % (species, t, sex)].keys():
            ...
            if relocate_distance > 0:
                ...
                if duplicates_exist:
                    ds_dup = ds_set.copy()
                    pop_add = numpy.bincount(bins, ds_set)[1:]
...
def minimum_viable_population(self, population, min_pop, area, filter_std=3):
    ...
    labels, num = ndimage.label(regions < breakpoint, numpy.ones(shape=(3, 3)))
    areas = ndimage.sum(numpy.ones(shape=labels.shape) * (self.csx * self.csy), labels, numpy.arange(num) + 1)
    pops = ndimage.sum(population, labels, numpy.arange(num) + 1)
    takeLabels = (numpy.arange(num) + 1)[(pops < min_pop) & (areas >= area)]
    indices = numpy.argsort(labels.ravel())
    bins = numpy.bincount(labels.ravel())
    indices = numpy.split(indices.ravel(), numpy.cumsum(bins[bins > 0][:-1]))
    indices = dict(zip(numpy.unique(labels.ravel()), indices))
    output = numpy.ones(shape=labels.ravel().shape, dtype='bool')
...
def convolve(a, kernel):
    kernel = numpy.atleast_2d(kernel)

    if kernel.size == 1:
        return a * numpy.squeeze(kernel)
    ...
    a_padded = numpy.pad(output, padding, 'constant')
